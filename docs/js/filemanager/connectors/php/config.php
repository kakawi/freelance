...
$ok = 0;
switch($_SESSION['user']['cur_page'])
{
    case('dbarticles'):
						$Config['UserFilesPath'] = '/documents/users/'.$_SESSION['user']['login'].'/';
						$ok = 1;
						break;
    case('news'):
	                    $Config['UserFilesPath'] = '/documents/news/';
						$ok = 1;
						break;
}
if($ok == 0) $Config['UserFilesPath'] = '/documents/';

// Путь к самому FileManager
$condir=strtr(dirname(__DIR__),'\\', '/');
$Config['FileManagerAbsolutePath'] = substr($condir,0,strrpos($condir,'/'));
$Config['FileManagerPath'] = str_ireplace(strtr($_SERVER["DOCUMENT_ROOT"],'\\', '/'),'',$Config['FileManagerAbsolutePath']);

// Skins
$Config['Skins']['Dir'] = 'skins';		// Папка со скинами
$Config['Skins']['AbsolutePath'] = $Config['FileManagerAbsolutePath'].'/'.$Config['Skins']['Dir'];	// Абсолютный путь к папке со скинами
$Config['Skins']['Path'] = $Config['FileManagerPath'].'/'.$Config['Skins']['Dir'];	// Относительный путь к папке со скинами
$Config['Skins']['Skins'] = array('Fatcow','Moono','MoonoBlue','Tango');	// название скина (должно совпадать с названием папки где лежит скин)
$Config['Skins']['Skin'] = 'Fatcow';		// Текущий скин
...
if (isset($_GET['js_conf'])) {
	$Config_js = array('Delete', 'ThumbList', 'ThumbListSize', 'ThumbCreate',
		...
		'Skins.Dir','Skins.Path','Skins.Skin','Skins.Skins');
